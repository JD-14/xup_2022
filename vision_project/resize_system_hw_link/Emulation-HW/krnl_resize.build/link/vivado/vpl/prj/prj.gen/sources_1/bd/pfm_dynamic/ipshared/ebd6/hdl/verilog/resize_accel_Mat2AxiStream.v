// ==============================================================
// RTL generated by Vitis HLS - High-Level Synthesis from C, C++ and OpenCL v2021.1 (64-bit)
// Version: 2021.1
// Copyright (C) Copyright 1986-2021 Xilinx, Inc. All Rights Reserved.
// 
// ===========================================================

`timescale 1 ns / 1 ps 

module resize_accel_Mat2AxiStream (
        out_mat_421_dout,
        out_mat_421_empty_n,
        out_mat_421_read,
        ldata1_din,
        ldata1_full_n,
        ldata1_write,
        rows,
        cols,
        ap_clk,
        ap_rst,
        ap_start,
        rows_ap_vld,
        cols_ap_vld,
        ap_done,
        ap_ready,
        ap_idle,
        ap_continue,
        ap_ext_blocking_n,
        ap_str_blocking_n,
        ap_int_blocking_n
);


input  [7:0] out_mat_421_dout;
input   out_mat_421_empty_n;
output   out_mat_421_read;
output  [127:0] ldata1_din;
input   ldata1_full_n;
output   ldata1_write;
input  [15:0] rows;
input  [31:0] cols;
input   ap_clk;
input   ap_rst;
input   ap_start;
input   rows_ap_vld;
input   cols_ap_vld;
output   ap_done;
output   ap_ready;
output   ap_idle;
input   ap_continue;
output   ap_ext_blocking_n;
output   ap_str_blocking_n;
output   ap_int_blocking_n;

wire    last_blk_pxl_width_1_U0_ap_start;
wire    last_blk_pxl_width_1_U0_ap_done;
wire    last_blk_pxl_width_1_U0_ap_continue;
wire    last_blk_pxl_width_1_U0_ap_idle;
wire    last_blk_pxl_width_1_U0_ap_ready;
wire   [3:0] last_blk_pxl_width_1_U0_return_r;
wire    last_blk_pxl_width_1_U0_ap_ext_blocking_n;
wire    last_blk_pxl_width_1_U0_ap_str_blocking_n;
wire    last_blk_pxl_width_1_U0_ap_int_blocking_n;
wire    last_blk_width_channel_full_n;
wire    MatStream2AxiStream_2_U0_ap_start;
wire    MatStream2AxiStream_2_U0_ap_done;
wire    MatStream2AxiStream_2_U0_ap_continue;
wire    MatStream2AxiStream_2_U0_ap_idle;
wire    MatStream2AxiStream_2_U0_ap_ready;
wire    MatStream2AxiStream_2_U0_out_mat_421_read;
wire   [127:0] MatStream2AxiStream_2_U0_ldata1_din;
wire    MatStream2AxiStream_2_U0_ldata1_write;
wire    MatStream2AxiStream_2_U0_ap_ext_blocking_n;
wire    MatStream2AxiStream_2_U0_ap_str_blocking_n;
wire    MatStream2AxiStream_2_U0_ap_int_blocking_n;
wire   [3:0] last_blk_width_channel_dout;
wire    last_blk_width_channel_empty_n;
wire    ap_sync_ready;
reg    ap_sync_reg_last_blk_pxl_width_1_U0_ap_ready;
wire    ap_sync_last_blk_pxl_width_1_U0_ap_ready;
reg    ap_sync_reg_MatStream2AxiStream_2_U0_ap_ready;
wire    ap_sync_MatStream2AxiStream_2_U0_ap_ready;
wire    ap_ext_blocking_cur_n;
wire    ap_str_blocking_cur_n;
wire    ap_int_blocking_cur_n;
wire    ap_ext_blocking_sub_n;
wire    ap_str_blocking_sub_n;
wire    ap_int_blocking_sub_n;
wire    ap_ce_reg;

// power-on initialization
initial begin
#0 ap_sync_reg_last_blk_pxl_width_1_U0_ap_ready = 1'b0;
#0 ap_sync_reg_MatStream2AxiStream_2_U0_ap_ready = 1'b0;
end

resize_accel_last_blk_pxl_width_1 last_blk_pxl_width_1_U0(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(last_blk_pxl_width_1_U0_ap_start),
    .ap_done(last_blk_pxl_width_1_U0_ap_done),
    .ap_continue(last_blk_pxl_width_1_U0_ap_continue),
    .ap_idle(last_blk_pxl_width_1_U0_ap_idle),
    .ap_ready(last_blk_pxl_width_1_U0_ap_ready),
    .return_r(last_blk_pxl_width_1_U0_return_r),
    .ap_ext_blocking_n(last_blk_pxl_width_1_U0_ap_ext_blocking_n),
    .ap_str_blocking_n(last_blk_pxl_width_1_U0_ap_str_blocking_n),
    .ap_int_blocking_n(last_blk_pxl_width_1_U0_ap_int_blocking_n)
);

resize_accel_MatStream2AxiStream_2_s MatStream2AxiStream_2_U0(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(MatStream2AxiStream_2_U0_ap_start),
    .ap_done(MatStream2AxiStream_2_U0_ap_done),
    .ap_continue(MatStream2AxiStream_2_U0_ap_continue),
    .ap_idle(MatStream2AxiStream_2_U0_ap_idle),
    .ap_ready(MatStream2AxiStream_2_U0_ap_ready),
    .out_mat_421_dout(out_mat_421_dout),
    .out_mat_421_empty_n(out_mat_421_empty_n),
    .out_mat_421_read(MatStream2AxiStream_2_U0_out_mat_421_read),
    .ldata1_din(MatStream2AxiStream_2_U0_ldata1_din),
    .ldata1_full_n(ldata1_full_n),
    .ldata1_write(MatStream2AxiStream_2_U0_ldata1_write),
    .rows(rows),
    .cols_bound_per_npc(cols),
    .last_blk_width(last_blk_width_channel_dout),
    .ap_ext_blocking_n(MatStream2AxiStream_2_U0_ap_ext_blocking_n),
    .ap_str_blocking_n(MatStream2AxiStream_2_U0_ap_str_blocking_n),
    .ap_int_blocking_n(MatStream2AxiStream_2_U0_ap_int_blocking_n)
);

resize_accel_fifo_w4_d2_S_x last_blk_width_channel_U(
    .clk(ap_clk),
    .reset(ap_rst),
    .if_read_ce(1'b1),
    .if_write_ce(1'b1),
    .if_din(last_blk_pxl_width_1_U0_return_r),
    .if_full_n(last_blk_width_channel_full_n),
    .if_write(last_blk_pxl_width_1_U0_ap_done),
    .if_dout(last_blk_width_channel_dout),
    .if_empty_n(last_blk_width_channel_empty_n),
    .if_read(MatStream2AxiStream_2_U0_ap_ready)
);

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_sync_reg_MatStream2AxiStream_2_U0_ap_ready <= 1'b0;
    end else begin
        if (((ap_sync_ready & ap_start) == 1'b1)) begin
            ap_sync_reg_MatStream2AxiStream_2_U0_ap_ready <= 1'b0;
        end else begin
            ap_sync_reg_MatStream2AxiStream_2_U0_ap_ready <= ap_sync_MatStream2AxiStream_2_U0_ap_ready;
        end
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_sync_reg_last_blk_pxl_width_1_U0_ap_ready <= 1'b0;
    end else begin
        if (((ap_sync_ready & ap_start) == 1'b1)) begin
            ap_sync_reg_last_blk_pxl_width_1_U0_ap_ready <= 1'b0;
        end else begin
            ap_sync_reg_last_blk_pxl_width_1_U0_ap_ready <= ap_sync_last_blk_pxl_width_1_U0_ap_ready;
        end
    end
end

assign MatStream2AxiStream_2_U0_ap_continue = ap_continue;

assign MatStream2AxiStream_2_U0_ap_start = (last_blk_width_channel_empty_n & (ap_sync_reg_MatStream2AxiStream_2_U0_ap_ready ^ 1'b1) & ap_start);

assign ap_done = MatStream2AxiStream_2_U0_ap_done;

assign ap_ext_blocking_cur_n = 1'b1;

assign ap_ext_blocking_n = (ap_ext_blocking_sub_n & ap_ext_blocking_cur_n);

assign ap_ext_blocking_sub_n = (last_blk_pxl_width_1_U0_ap_ext_blocking_n & MatStream2AxiStream_2_U0_ap_ext_blocking_n);

assign ap_idle = (last_blk_pxl_width_1_U0_ap_idle & (last_blk_width_channel_empty_n ^ 1'b1) & MatStream2AxiStream_2_U0_ap_idle);

assign ap_int_blocking_cur_n = 1'b1;

assign ap_int_blocking_n = (ap_int_blocking_sub_n & ap_int_blocking_cur_n);

assign ap_int_blocking_sub_n = (last_blk_pxl_width_1_U0_ap_int_blocking_n & MatStream2AxiStream_2_U0_ap_int_blocking_n);

assign ap_ready = ap_sync_ready;

assign ap_str_blocking_cur_n = 1'b1;

assign ap_str_blocking_n = (ap_str_blocking_sub_n & ap_str_blocking_cur_n);

assign ap_str_blocking_sub_n = (last_blk_pxl_width_1_U0_ap_str_blocking_n & MatStream2AxiStream_2_U0_ap_str_blocking_n);

assign ap_sync_MatStream2AxiStream_2_U0_ap_ready = (ap_sync_reg_MatStream2AxiStream_2_U0_ap_ready | MatStream2AxiStream_2_U0_ap_ready);

assign ap_sync_last_blk_pxl_width_1_U0_ap_ready = (last_blk_pxl_width_1_U0_ap_ready | ap_sync_reg_last_blk_pxl_width_1_U0_ap_ready);

assign ap_sync_ready = (ap_sync_last_blk_pxl_width_1_U0_ap_ready & ap_sync_MatStream2AxiStream_2_U0_ap_ready);

assign last_blk_pxl_width_1_U0_ap_continue = last_blk_width_channel_full_n;

assign last_blk_pxl_width_1_U0_ap_start = ((ap_sync_reg_last_blk_pxl_width_1_U0_ap_ready ^ 1'b1) & ap_start);

assign ldata1_din = MatStream2AxiStream_2_U0_ldata1_din;

assign ldata1_write = MatStream2AxiStream_2_U0_ldata1_write;

assign out_mat_421_read = MatStream2AxiStream_2_U0_out_mat_421_read;

endmodule //resize_accel_Mat2AxiStream
