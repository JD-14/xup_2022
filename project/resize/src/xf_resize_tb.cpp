...
int main(int argc, char** argv) {
    ...
    cv::Mat img, result_hls, result_ocv, error, out_thrsh_img, ocv_ref, in_gray, diff;
    ...
    //thresholding matrices
    ocv_ref.create(out_height, out_width, in_img.depth());
    out_thrsh_img.create(out_height, out_width, in_img.depth());
    diff.create(out_height, out_width, in_img.depth());
...
// OpenCL section:
    ...
    OCL_CHECK(err, err = krnl.setArg(6, thresh));
    OCL_CHECK(err, err = krnl.setArg(7, maxval));
...
//////////////////////////////////////////
    ...
    cv::Mat result_hw(out_height, out_width, img.type(), (void *)buf_from_device);
...
/*OpenCV resize function*/
    ...
    cv::absdiff(result_hw, result_ocv, error);
    float err_per;
